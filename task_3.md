# Инкремент 3 — проверка публичного Railway-деплоя

## Предпосылка

Инкремент 2 слит и развёрнут хотя бы один раз. Публичный `/api/health` возвращает Git SHA.
...
Скрипт должен тестироваться против локального production-контейнера из инкремента 2:
...
ненулевым кодом. Команды адаптировать к точному имени build argument из инкремента 2.
